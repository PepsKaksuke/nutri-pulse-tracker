...
interface FoodCardProps {
  food: Food;
  isSelected?: boolean;
  onSelect?: (food: Food, quantity?: string) => void;
  onClick?: () => void;
  className?: string;
...
export const FoodCard: React.FC<FoodCardProps> = ({
  ...
  const handleSelectClick = (e: React.MouseEvent) => {
    ...
    if (onSelect) {
      if (isSelected) {
        // Si l'aliment est déjà sélectionné, on le retire directement
        onSelect(food);
      } else {
        // Sinon, on ouvre le dialogue de sélection de quantité
        setDialogOpen(true);
      }
    }
  };

  const handleConfirmQuantity = (food: Food, quantity: string) => {
    if (onSelect) {
